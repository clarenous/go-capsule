...
// from  O(N^2) to O(N).
func calcHashPrevOuts(tx *types.Tx) types.Hash {
	var b bytes.Buffer
...
// hashing computation, reducing the complexity of validating SigHashAll inputs
// from O(N^2) to O(N).
func calcHashSequence(tx *types.Tx) types.Hash {
	var b bytes.Buffer
	for _, in := range tx.TxIn {
		var buf [4]byte
...
func calcHashOutputs(tx *types.Tx) types.Hash {
	var b bytes.Buffer
...
func calcWitnessSignatureHash(subScript []parsedOpcode, sigHashes *TxSigHashes,
	hashType SigHashType, tx *types.Tx, idx int, amt int64) ([]byte, error) {
	...
	return types.DoubleHashB(sigHash.Bytes()), nil
}
...
func CalcWitnessSigHash(script []byte, sigHashes *TxSigHashes, hType SigHashType,
	tx *types.Tx, idx int, amt int64) ([]byte, error) {
...
func shallowCopyTx(tx *types.Tx) types.Tx {
...
// verification.
func CalcSignatureHash(script []byte, hashType SigHashType, tx *types.Tx, idx int) ([]byte, error) {
	parsedScript, err := parseScript(script)
	if err != nil {
		return nil, fmt.Errorf("cannot parse output script: %v", err)
	}
...
// engine instance, calculate the signature hash to be used for signing and
// verification.
func calcSignatureHash(script []parsedOpcode, hashType SigHashType, tx *types.Tx, idx int) []byte {
