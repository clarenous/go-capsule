...
type TxSigHashes struct {
	HashPrevOuts types.Hash
	HashSequence types.Hash
	HashOutputs  types.Hash
}

// NewTxSigHashes computes, and returns the cached sighashes of the given
// transaction.
func NewTxSigHashes(tx *types.Tx) *TxSigHashes {
	return &TxSigHashes{
...
type HashCache struct {
	sigHashes map[types.Hash]*TxSigHashes
...
func NewHashCache(maxSize uint) *HashCache {
	return &HashCache{
		sigHashes: make(map[types.Hash]*TxSigHashes, maxSize),
	}
}

// AddSigHashes computes, then adds the partial sighashes for the passed
// transaction.
func (h *HashCache) AddSigHashes(tx *types.Tx) {
	h.Lock()
...
func (h *HashCache) ContainsHashes(txid *types.Hash) bool {
	h.RLock()
...
func (h *HashCache) GetSigHashes(txid *types.Hash) (*TxSigHashes, bool) {
	h.RLock()
...
func (h *HashCache) PurgeSigHashes(txid *types.Hash) {
	h.Lock()
