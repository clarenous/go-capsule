...
func opcodeCheckLockTimeVerify(op *parsedOpcode, vm *Engine) error {
	...
	// setting its sequence to the maximum value (consensus.MaxTxInSequenceNum).  This
	...
	if vm.tx.TxIn[vm.txIdx].Sequence == consensus.MaxTxInSequenceNum {
...
func opcodeCheckSequenceVerify(op *parsedOpcode, vm *Engine) error {
	...
	if sequence&int64(consensus.SequenceLockTimeDisabled) != 0 {
		return nil
	}
	...
	if txSequence&int64(consensus.SequenceLockTimeDisabled) != 0 {
	...
	lockTimeMask := int64(consensus.SequenceLockTimeIsSeconds |
		consensus.SequenceLockTimeMask)
	return verifyLockTime(txSequence&lockTimeMask,
		consensus.SequenceLockTimeIsSeconds, sequence&lockTimeMask)
}
