...
func GetTxMerkleTreeProof(txs []*Tx, relatedTxs []*Tx) ([]*Hash, []uint8) {
	var rawDatas []merkleNode
	var relatedRawDatas []merkleNode
	for _, tx := range txs {
		rawDatas = append(rawDatas, tx.Hash().Ptr())
	}
	for _, relatedTx := range relatedTxs {
		relatedRawDatas = append(relatedRawDatas, relatedTx.Hash().Ptr())
	}
